...
f32 random_float(void) { return 0.0f; }
u16 random_u16(void) { return 0; }
...
const BehaviorScript bhvCarrySomething3[] = { 0 };
const BehaviorScript bhvCarrySomething4[] = { 0 };
const BehaviorScript bhvCarrySomething5[] = { 0 };
...
s32 load_patchable_table(struct MarioAnimation *a, u32 index) {
    ...
    if (index < gMarioNumAnims) {
        ...
        if (a->currentAnimAddr != (u8*)addr) {
            memcpy(a->targetAnim, addr, sizeof(struct Animation));
            a->currentAnimAddr = (u8*)addr;
        }
    }
    return ret;
}
...
EXPORT void ADDCALL init(FindFloorHandler_t *floorHandler, FindCeilHandler_t *ceilHandler, FindWallHandler_t *wallHandler, FindWaterLevelHandler_t *waterHandler) {
    ...
    gMarioState->animation->targetAnim = &gTargetAnim;
    gMarioState->statusForCamera = &gPlayerCameraState[0];
    gMarioState->health = 0x880;
    gMarioState->marioObj->header.gfx.animInfo.curAnim = gMarioState->animation->targetAnim;
